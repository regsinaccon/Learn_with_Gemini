...
def markdown_to_kivy_markup(md_text):
...
class Home(Screen):
    def __init__(self, **kw):
        ...
        self.search_button = Button(text = 'search',font_size = 40,size_hint=(0.3,1))
    ...
    def search_word_onGe(self,instance):
        ask = kit()
        question = self.search_input
        global searching_word_explanation
        searching_word_explanation = ask.add_and_show_word(question)
        print(searching_word_explanation)
...
class MyApp(App):
    def build(self):
        ...
        # md_text = "**Hello**, *world*!"
        # kivy_markup_text = markdown_to_kivy_markup(md_text)
        # return Label(text=kivy_markup_text, markup=True,font_size="100sp")
        return sm 
